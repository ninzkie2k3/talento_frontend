...
export default function AddBook() {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
        // Get performer IDs from the portfolio data
        const performerIds = performers.map(performer => performer.performer_portfolio?.id);
        
        // Verify we have valid IDs
        if (!performerIds.every(id => id)) {
            throw new Error("Missing performer portfolio IDs");
        }
        ...
        if (error.response?.data?.error) {
            toast.error(Array.isArray(error.response.data.error) 
                ? error.response.data.error.join(', ')
                : error.response.data.error
            );
        } else {
            toast.error(error.message || 'There was an error booking the performers. Please check your data and try again.');
        }
    } finally {
